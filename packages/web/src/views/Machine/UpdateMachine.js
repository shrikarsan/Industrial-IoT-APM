...
const UpdateMachine = () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        formik.setFieldValue(
          "purchasedDate",
          res.data.data.purchasedDate.substring(0, 10)
        );
        formik.setFieldValue(
          "lastMaintenance",
          res.data.data.lastMaintenance.substring(0, 10)
        );
        if (res.data.data.actuatorId !== undefined) {
          formik.setFieldValue("isActuatorPresent", true);
        }
        formik.setFieldValue("actuatorId", res.data.data.actuatorId);
        formik.setFieldValue("actuatorName", res.data.data.actuatorName);
      }
    } catch (err) {
  ...
  const updateMachine = async (values, formikActions) => {
    const res = await client.put("/machines", {
      ...values,
    });
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, formikActions) => {
      updateMachine(values, formikActions);
    },
  });

  return (
    <Layout title="Update Machine">
